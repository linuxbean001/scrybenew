<template>
  <DashboardContainer>
    <div class="mydiv">
      <h1 class="text-h2">Chat Application</h1>
...
export default {
  data() {
    return {
      ...
      botMsgs: "test",
      botImg:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIG10QtjFI4Hex516TJ7wpzvDmYjpK97dGgZLqlzc7FAC5igMPICB8zFrTzYinqkfBT0Y&usqp=CAU",
      personImg:
  ...
  methods: {
    handleSubmit() {
      ...
      fetch(`${process.env.VUE_APP_API_URL}/api/query`, requestData)
        .then((response) => response.json())
        .then((result) => {
          // Set the API response message in botMsgs
    ...
    botResponse() {
      const r = this.random(0, this.botMsgs.length - 1);
      const msgText = this.botMsgs[r];
      const delay = msgText.split(" ").length * 100;

      setTimeout(() => {
        this.appendMessage(this.botName, this.botImg, "left", msgText);
      }, delay);
    },
    formatDate(date) {
